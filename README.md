...
- **Website** (`docs/index.html`): trilingual single page (es / en / zh-Hant) with the current week's lists, both scenarios week by week, the protocol, every review round with counts derived from Astra's JSON outputs, data coverage, limits and next steps. Figures come from `docs/site/data.json`, exported by `scripts/export_site_data.py` from the backtest JSONs and the review outputs; the explanatory texts in the three languages are the author's and are checked by Astra against the artefacts.

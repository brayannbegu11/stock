...
`python -m pytest -q -p no:cacheprovider` ejecuta las pruebas de aceptación (recuento vigente en `README.md`) que cubren PIT-01 a PIT-12, UNI-01/02/03/05/07, TXT-05/07, SIM-01/02/03/05/06/07/08/09/10/11/12, STA-01/04, la aritmética de fricción de 0,585 % y los contraejemplos reproducibles de las rondas 1 a 8 de Astra (`docs/informes/03_…` a `12_respuesta_ronda8_astra.md`). Todo con datos sintéticos explícitos o con las capturas oficiales del calendario. Ninguna prueba usa red. La primera corrida real de `scripts/capture_daily.py` (37 endpoints, 0 fallos) está registrada en `data/audit/manifest_2026-09-09.jsonl`.
...
2. ~~Rondas 3 a 8~~ — hechas: 17 + 14 + 12 + 10 + 10 + 13 hallazgos, corregidos con prueba; lotes con propietario, sello sobre la predicción canónica + `packet_hash` (el sello se recalcula desde el archivo; el paquete se recupera de un registro y se readmite documento a documento), registro fijo (sólo lectura, vacío) de verificadores de producción, corte semanal estricto, identidad símbolo+fecha de alta, universo del tablero principal (informes 05-09 y 12).
3. Activar la captura diaria (decisión del usuario: `scripts/register_daily_capture.ps1`) y medir latencias dos semanas.
4. ~~Poblar el maestro con TWSE/TPEx/FinMind~~ — hecho para el censo vigente (informe 10); pendientes el maestro histórico (retiradas sin fecha de alta) y la prueba de equivalencia de unidades emparejada.

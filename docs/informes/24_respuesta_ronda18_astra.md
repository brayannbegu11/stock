...
| R18-08 | media | Exportador: conserva `final_valuation` (patrimonio, marcas, fecha, sesión de precios, fracciones sin resolver) y los campos planos `final_flags`, `final_valued_at`, `final_prices_session`; el sitio marca el patrimonio final como PROVISIONAL cuando hay marcas y muestra la fecha y sesión de la valoración final. | `test_r18_08_export_keeps_final_valuation_conditions` |
| R18-09 | baja | De las dos opciones propuestas se toma la segunda: el informe 23 §3 remite al recuento derivado (`review_stats.new_total`) y README deja de citar un número fijo de rondas, para que no vuelvan a quedar desfasados. La prueba de Astra que exige números literales no aplica a esta opción. | textual |
| R18-10 | baja | Informe 22 §2 incorpora P7 como adenda (propuesta en el prompt de la ronda 17, evaluada por Astra en esa ronda) y el informe 15b remite a esa adenda y al informe 23 §2. | textual |

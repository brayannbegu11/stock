...
| R06-09 | alta | Un puesto `exited` con residuo se reintenta en salidas posteriores; si el residuo se ha convertido en lote entero, se vende y el resultado se actualiza. *(Garantía acotada tras R07-06/07: un reintento bloqueado o fuera de orden cronológico no estaba cubierto; véase informe 09.)* | `test_r06_09_residual_turned_round_lot_is_sold_on_retry` |
...
2. **Recuperación del paquete y del calendario acreditados.** *(Redacción corregida tras R07-10.)* En esta ronda el evaluador validaba la predicción archivada sólo contra el esquema e imponía restricciones estructurales sobre el plazo **declarado** en el sobre (dentro de la semana objetivo), sin recuperar el paquete ni el calendario: el plazo seguía dependiendo del `deadline_at` archivado y el paquete no se acreditaba. Astra lo demostró en R07-01..R07-04; la corrección (registro de paquetes por `packet_hash`, calendario obligatorio y re-validación completa) está en `09_respuesta_ronda7_astra.md`.

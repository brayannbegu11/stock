...
- Las etiquetas de entrenamiento son rentabilidades apertura→cierre de semanas del protocolo cuya última
  barra tiene ``available_at`` ≤ T. Una semana cuyo viernes aún no está disponible en T no se etiqueta.
- El modelo se reentrena con cadencia declarada (``retrain_every_weeks``) y sólo con filas anteriores al corte.

El objetivo es el **rango** cruzado de la rentabilidad semanal (0..1 dentro de cada semana), no el nivel:
reduce el peso de valores extremos y es lo que la cesta de cinco puestos necesita (ordenar, no estimar).
Sin fundamentales ni noticias todavía: es un modelo de precios/volumen, deliberadamente modesto.
"""
...
MODEL_ID = "q1:tabular_ridge_lgbm_rank_v1"
...
def _log_ret(a: Decimal, b: Decimal) -> Optional[float]:
...
class TrainingRow:
    ...
    label: float                 # rentabilidad apertura→cierre de la semana siguiente al corte
    label_known_at: datetime     # available_at de la última barra usada para la etiqueta


def weekly_label(bars_by_session: Mapping[date, BarLike], entry_s: date, exit_s: date) -> Optional[tuple[float, datetime]]:
    a, b = bars_by_session.get(entry_s), bars_by_session.get(exit_s)
    if a is None or b is None or a.open <= 0 or b.close <= 0:
        return None
    return float(b.close) / float(a.open) - 1.0, b.available_at
...
class RidgeRank:
    ...
    def fit(self, X: Sequence[Sequence[float]], y: Sequence[float]) -> "RidgeRank":
        ...
        yc = [v - ym for v in y]
        # (ZᵀZ + αI) w = Zᵀy  — resolución por eliminación de Gauss (p es pequeño)
...
def build_training_rows(
    ...
    *,
    ...
    cache: Optional[dict[tuple[datetime, str], Optional[TrainingRow]]] = None,
) -> list[TrainingRow]:
    ...
    for cutoff in cutoffs:
        ...
        for sec, bars in bars_by_security.items():
            ...
            if key not in cache:
                ...
                lab = weekly_label({b.session: b for b in bars}, entry_s, exit_s) if feats else None
                cache[key] = TrainingRow(cutoff, sec, feats, lab[0], lab[1]) if (feats and lab) else None
            row = cache[key]
...
def fit_q1(rows: Sequence[TrainingRow], *, trained_at: datetime, min_weeks: int = 52, seed: int = 20260909) -> Optional[Q1Model]:
    ...
    for cutoff, wrows in sorted(by_week.items()):
        ...
        for r, rk in zip(wrows, ranks):
            ...
            y.append(rk)
    ridge = RidgeRank(alpha=float(len(X)) * 0.01).fit(X, y)
    lgbm = None
    try:
        import lightgbm as lgb
        lgbm = lgb.LGBMRegressor(n_estimators=200, learning_rate=0.03, num_leaves=15, min_child_samples=50, subsample=0.8,
                                 subsample_freq=1, colsample_bytree=0.8, reg_lambda=5.0, random_state=seed, verbose=-1)
        lgbm.fit(X, y)
    except Exception:
        lgbm = None
    weeks = sorted(by_week)
    iso0, iso1 = weeks[0].date().isocalendar(), weeks[-1].date().isocalendar()
    manifest = f"{MODEL_ID}|rows={len(X)}|weeks={len(weeks)}|labels={iso0[0]}-W{iso0[1]:02d}..{iso1[0]}-W{iso1[1]:02d}|lgbm={'yes' if lgbm else 'no'}"
    return Q1Model(trained_at=trained_at, n_rows=len(X), n_weeks=len(weeks), first_label_week=f"{iso0[0]}-W{iso0[1]:02d}",
                   last_label_week=f"{iso1[0]}-W{iso1[1]:02d}", ridge=ridge, lgbm=lgbm, training_manifest_id=manifest)
